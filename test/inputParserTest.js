...
import {parseInputForAcknowledge, parseInputForDowntime} from '../src/inputParser'
...
describe('parse', () => {
    describe('acknowledgement', () => {
        ...
        it('should correctly parse an acknowledgement', () => {
            let parsed = parseInputForAcknowledge('acknowledge Puppet run result across all exchanges on some-host');
    ...
    describe('downtime', () => {
        it('should fail to parse with an error', () => {
            let parsed = parseInputForDowntime('schedule downtime for not a valid anything');

            expect(parsed).to.have.property('error');
        });

        it('should correctly parse a downtime command with a service', () => {
            let parsed = parseInputForDowntime('schedule downtime for Puppet run result across all exchanges on some-host for 100');
        ...
        it('should correctly parse a downtime command without a service', () => {
            let parsed = parseInputForDowntime('schedule downtime on some-host for 100');
