...
describe('handleDowntime', () => {
    ...
    it('should send the correct request and reply with the correct message', done => {
        const inputText = {text: 'schedule downtime for Check auction log archiving on S3 on some-host for 10'};
        const bot = {
            reply: function (orginalMessage, message) {
                expect(message).to.equal('Service:Check auction log archiving on S3, Host:some-host');
                done();
            }
        };
